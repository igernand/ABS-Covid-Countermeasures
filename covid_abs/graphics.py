...
def update_graphRL(sim, agent, ax, linhas1, ax1, linhas2, ax2, statistics):
    """
    Execute an iteration of the simulation and update the animation graphics
    ...
    if sim.iteration % 240 == 0:
        ...
        #observation_, reward = sim.execute(action)
        # set new mode 
...
def execute_graphsimulationRL(sim, agent, **kwargs):
    from matplotlib.ticker import (MultipleLocator, FormatStrFormatter,
                                   AutoMinorLocator)
    ...
    animate = lambda i: update_graphRL(sim, agent, ax[0], linhas1, ax[1], linhas2, ax[2], statistics)
