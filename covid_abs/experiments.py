...
def plot_batch_results(df, health_metrics=('Susceptible', 'Infected', 'Symptomatic', 'Severe', 'Recovered_Immune', 'Death'),
...
def plot_graph_batch_results(df, **kwargs):
    ...
    health_metrics=('Susceptible', 'Infected', 'Symptomatic', 'Severe', 'Recovered_Immune', 'Death')
    ecom_metrics=('Q1', 'Q2', 'Q3', 'Q4', 'Q5','Business','Government')

    health_legend={'Susceptible': '$S_t$ - Susceptible', 'Infected':'$I_t$ - Infected', 
                   'Symptomatic':'$Sy$ - Symptomatic', 'Severe':'$I^S_t$ - Severe', 
...
def batch_experimentRL(experiments, iterations, file, simulation_type=Simulation, **kwargs):
    """
    Execute several simulations with the same parameters and store the average statistics by iteration
    ...
    RLagent = RLAgent(gamma=0.99, epsilon=1.0, batch_size=64, n_actions=5, eps_end=0.01, input_dims=[5], lr=0.003)
    FILE = "modelinfo.pth"
    ...
    for experiment in range(experiments):
        try:
            ...
            for it in range(iterations):
                if verbose == 'iterations':
                    print('Experiment {}\tIteration {}'.format(experiment, it))
                if sim.iteration % 240 == 0:
                    action = agent.choose_action(sim.observation)
                    sim.change_mode(action)
                    sim.check_mode()
                sim.execute()
                sim.statistics = None
                statistics = sim.get_statistics(kind='all')
